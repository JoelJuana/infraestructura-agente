# data-layer.tf
# Define la capa de persistencia (GCS, Firestore) y la inteligencia (Vertex AI).

# --------------------------------------------------------------------------------------
# 1. GOOGLE CLOUD STORAGE (GCS)
# --------------------------------------------------------------------------------------

resource "google_storage_bucket" "rag_raw_docs" {
    for_each = var.gcs_bucket_config
    # Nomenclatura profesional: [app]-[env]-bkt-[suffix]-[project_id]
    name     = "${local.resource_prefix}-bkt-${each.value.name_suffix}-${var.project_id}" 
    location = var.region
    force_destroy = each.value.force_destroy
    uniform_bucket_level_access = true
    depends_on = [google_project_service.apis]
}

# --------------------------------------------------------------------------------------
# 2. FIRESTORE
# --------------------------------------------------------------------------------------

resource "google_firestore_database" "firestore_db" {
    project     = var.project_id
    name        = "(default)"
    location_id = var.firestore_location_id 
    type        = "FIRESTORE_NATIVE"
    depends_on  = [google_project_service.apis]
}

# --------------------------------------------------------------------------------------
# 3. VERTEX AI (ÍNDICES VECTORIALES)
# --------------------------------------------------------------------------------------

resource "google_vertex_ai_index" "rag_index" {
    project      = var.project_id
    region       = var.region
    # Nomenclatura profesional: [app]-[env]-ai-index
    display_name = "${local.resource_prefix}-ai-index"
    description  = "Índice de vectores para ${var.application_display_name} en ${upper(var.environment)}."
    metadata {
        contents {
            dimensions = var.rag_index_dimensions
            config {
                distance_measure_type = "DOT_PRODUCT"
            }
        }
    }
    depends_on = [google_project_service.apis]
}

resource "google_vertex_ai_index_endpoint" "rag_endpoint" {
    project      = var.project_id
    region       = var.region
    # Nomenclatura profesional: [app]-[env]-ai-endpoint
    display_name = "${local.resource_prefix}-ai-endpoint"
    description  = "Endpoint para consultas RAG del ${var.application_display_name}."
    depends_on = [google_vertex_ai_index.rag_index]
}